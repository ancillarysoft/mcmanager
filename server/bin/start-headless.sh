...
#> [ start-headless.sh ]
#>
#>    Initialize a headless Minecraft server
#>
...
#> USAGE:
#>
#>    start-headless.sh <OPTION>
#>    start-headless.sh <INPUT>
#>
...
#
    ...
    echo "Calling library function: mcmanager_server_start --headless" | mcmanager_server_logger -i

    if [[ "x${ops_version}" == "x" ]]; then
        mcmanager_server_start --headless
    else
        mcmanager_server_start --headless --version="${ops_version}"
    fi
